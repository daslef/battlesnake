...
const LeaderboardParticipant = forwardRef<HTMLTableRowElement, ILeaderboardParticipant>(
  function LeaderboardParticipant(props, ref) {
    ...
    return (
      <tr className="leaderboard__participant participant" ref={ref}>
        ...
        <td className="participant__team">{teamName}</td>
        <td className="participant__score participant__score--gold">
          <img
            src={digitOneSvg}
            alt=""
            className="participant__score__icon participant__score__icon--gold"
          />
        ...
        <td className="participant__score participant__score--final">
          <span className="participant__score__value participant__score__value--final">
            {Math.floor(score.total)}
          </span>
        </td>
      </tr>
    )
...
export default function Leaderboard() {
  ...
  const getSortedScore = useTournamentStore((store) => store.getSortedScore)

  const participants = useTournamentStore((store) => store.participants)
      ...
      <table>
        <tbody className="leaderboard__content" style={{ position: 'relative' }}>
          <FlipMove
            ...
            typeName={null}
          >
            {Object.entries(getSortedScore())?.map(([pSnakeName, participantScore]) => {
              const participant = participants.find(({ snakeName }) => snakeName === pSnakeName)
              return (
                <LeaderboardParticipant
                  key={`leaderboard_${participant?.snakeAuthor}`}
                  languageLogo={participant!.languageIcon}
                  teamName={participant!.snakeAuthor}
