...
func (settings Settings) GetRand(turn int) rules.Rand {
	...
	if settings.seed != 0 {
		return rules.NewSeedRand(settings.seed + int64(turn))
	}

	// Default to global random number generator if neither seed or rand are set.
	return rules.GlobalRand
}

func (settings Settings) WithRand(rand rules.Rand) Settings {
	settings.rand = rand
	return settings
}
