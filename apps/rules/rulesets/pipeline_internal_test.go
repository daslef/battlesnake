...
func TestPipelineRuleset(t *testing.T) {
	...
	require.Equal(t, rules.ErrorStageNotFound, pr.Err())
	...
	ended, _, err := pr.Execute(&rules.BoardState{}, nil)
	...
	r.RegisterPipelineStage("add_food", func(bs *rules.BoardState, s settings.Settings, sm []SnakeMove) (bool, error) {
		if IsInitialization(bs, s, sm) {
			return false, nil
		}
		bs.Food = append(bs.Food, rules.Point{X: 0, Y: 0})
		return false, nil
	})
	b := &rules.BoardState{}
...
func TestPipelineGlobals(t *testing.T) {
	...
	ended, next, err := p.Execute(&rules.BoardState{}, settings.Settings{}, nil)
	require.NoError(t, err)
