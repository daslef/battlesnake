...
func TestMaybeSpawnFoodZeroChance(t *testing.T) {
	r := getStandardRuleset(settings.NewSettingsWithParams(rules.ParamFoodSpawnChance, "0"))
	b := &rules.BoardState{
		Height: 11,
		Width:  11,
		Snakes: []rules.Snake{
			{Body: []rules.Point{{X: 1, Y: 0}, {X: 1, Y: 1}}},
			{Body: []rules.Point{{X: 0, Y: 1}, {X: 0, Y: 2}, {X: 0, Y: 3}}},
		},
		Food: []rules.Point{},
	}
	for i := 0; i < 1000; i++ {
...
func TestMaybeSpawnFoodHundredChance(t *testing.T) {
	r := getStandardRuleset(settings.NewSettingsWithParams(rules.ParamFoodSpawnChance, "100"))
	b := &rules.BoardState{
		Height: 11,
		Width:  11,
		Snakes: []rules.Snake{
...
func TestMaybeSpawnFoodHalfChance(t *testing.T) {
	tests := []struct {
		...
		{123, []rules.Point{}, 1},
		{12345, []rules.Point{}, 0},
		{456, []rules.Point{{X: 4, Y: 4}}, 1},
		{789, []rules.Point{{X: 4, Y: 4}}, 2},
		{511, []rules.Point{{X: 4, Y: 4}}, 1},
		{165, []rules.Point{{X: 4, Y: 4}}, 2},
	}

	r := getStandardRuleset(settings.NewSettingsWithParams(rules.ParamFoodSpawnChance, "50"))
	for _, test := range tests {
		b := &rules.BoardState{
			Height: 4,
			Width:  5,
